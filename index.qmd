...
```{R}
rmse_summaries <- data.frame(model_type = c("lm", "rpart"), 
                             rmse = (c(lm_rmse, rpart_rmse)))
rmse_summaries
...
```{R}
df <- read_csv("C:/Users/marc_/OneDrive - The Pennsylvania State University/RStudio/STAT380-hw-5-MarcVHughes/data/spambase.csv")
df <- df %>%
...
overview <- function(pred_class, true_class) {
  accuracy <- (tue_positives + true_negatives) / length(pred_class)
  error <- 1-accuracy
  ...
  false_positive_rate <- false_positives / (false_positives + ture_negatives)
  return(
    data.frame(
      accuracy = accuracy,
      error = error,
